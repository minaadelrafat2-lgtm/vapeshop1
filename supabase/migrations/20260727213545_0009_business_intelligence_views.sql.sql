...
## Overview
Adds 12 analytical views for the Reports module, plus a helper function for revenue analytics
with custom date range support. All views are read-only and update in real time as data changes.

## Views created
1. v_bi_sales_daily — daily sales aggregates (revenue, orders, items, avg order value)
2. v_bi_product_sales — per-product sales totals (best/worst sellers, revenue, qty sold)
3. v_bi_category_sales — per-category sales totals
4. v_bi_branch_sales — per-branch revenue and order counts
5. v_bi_employee_performance — per-employee sales metrics (via POS orders)
6. v_bi_inventory_value — inventory valuation at cost and retail
7. v_bi_low_stock — products at or below reorder level
8. v_bi_supplier_summary — supplier spend, PO counts, outstanding balances
9. v_bi_purchase_summary — purchase order totals by status
10. v_bi_expense_summary — expense totals by category and status
11. v_bi_top_customers — customers ranked by total spend and order count
12. v_bi_profit_analysis — revenue vs cost vs profit per product

## Functions
- get_revenue_analytics(start_date, end_date) — returns daily revenue within a custom date range
- get_product_sales_report(start_date, end_date) — product sales within a date range

## Security
- All views accessible to staff (is_staff()) via existing RLS on underlying tables.
