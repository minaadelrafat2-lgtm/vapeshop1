...
## Overview
Implements complete sales management with automatic inventory synchronization.

## New Tables
- `order_timeline` — immutable event log per order (created, paid, fulfilled, shipped, delivered, cancelled, returned, refunded)
- `order_returns` — return requests with line items and restocking status
- `order_refunds` — refund records (full/partial) linked to payments
- `branch_orders` — orders placed at/fulfilled by a specific branch (POS-ready foundation)

## Automation (database triggers)
1. Website orders automatically reduce inventory (place_order already does this; new trigger also logs timeline)
2. Cancelled orders restore inventory + log timeline event
3. Returned products restore inventory + log timeline event
4. Branch inventory updates automatically when branch orders are fulfilled
5. Warehouse inventory updates automatically via existing receive_purchase_order
6. Low-stock alerts auto-created when product drops below reorder_level
7. Notifications auto-created for: new orders, low stock, refunds, returns, payment failures

## Security
- RLS on all new tables.
- order_timeline: owner read (via order), staff full.
- order_returns / order_refunds: owner read (via order), staff full.
- branch_orders: staff only.
...
-- source constraint: website | pos | phone | branch_transfer
DO $$
BEGIN
...
-- order_returns: owner read, owner insert, staff full
...
-- order_return_items: owner read (via return), staff full
...
-- order_refunds: owner read (via order), staff full
...
-- Log an order timeline event (idempotent helper)
...
-- Cancel an order: restore inventory + log timeline + notify
...
-- Process a return: restore inventory + create refund + log timeline + notify
...
-- Issue a standalone refund (full or partial)
...
-- Create a notification (for staff/admin users or specific user)
...
-- Create a branch order (POS-ready)
...
-- Auto-log 'created' event when a new order is inserted
...
-- Auto-create low-stock alert when product stock drops below reorder_level
...
-- ============================================================
-- HELPER: format currency for messages (simple)
-- ============================================================
