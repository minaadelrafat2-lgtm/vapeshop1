...
## Overview
Fixes critical bugs in inventory logic, security policies, and adds missing indexes
identified during full codebase audit. All changes are additive and non-destructive.

## 1. Security Fixes
- is_staff() now recognizes all 11 seeded hierarchy roles in addition to legacy roles.
- cart_items INSERT/UPDATE policies tightened to prevent user_id injection.
- orders status CHECK constraints added.

## 2. Inventory Logic Fixes
- place_order() validates stock >= quantity, validates is_active, validates coupon.
- transfer_stock() actually moves stock between locations.
- receive_purchase_order() uses atomic ON CONFLICT DO UPDATE (no double-count).
- process_return() sets correct partial vs full refund status.
- issue_refund() sets correct partial vs full refund status, validates amount.
- cancel_order() validates order is not delivered.

## 3. New Functions
- receive_stock_transfer(p_transfer_id) — completes a transfer.
- place_customer_order() — atomic checkout with stock validation.

## 4. Performance — 25+ Missing Indexes
Adds indexes on all unindexed foreign keys and frequently-filtered columns.
*/
...
BEGIN
  ...
  -- Increment destination (atomic upsert)
