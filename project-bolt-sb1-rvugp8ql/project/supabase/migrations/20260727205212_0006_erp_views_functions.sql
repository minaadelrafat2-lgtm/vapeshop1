...
## Overview
Adds the business logic that makes inventory synchronize automatically:
- `receive_purchase_order()` — receives a PO, creates batches, updates inventory, logs transactions
- `record_supplier_payment()` — records a payment to a supplier
- `create_stock_alert()` — creates a stock alert
- `resolve_stock_alert()` — marks an alert resolved
- Views: inventory_valuation, branch_performance, supplier_outstanding, inventory_timeline, product_batches_detail, stock_alerts_detail

## Security
- All functions SECURITY DEFINER with fixed search_path.
...
SELECT
  b.id AS branch_id, b.name AS branch_name, b.code AS branch_code, b.is_active,
  coalesce(sum(o.grand_total) FILTER (WHERE o.status NOT IN ('cancelled','refunded')), 0) AS revenue,
  coalesce(count(o.id) FILTER (WHERE o.status NOT IN ('cancelled','refunded')), 0) AS order_count,
  coalesce(sum(e.amount) FILTER (WHERE e.status IN ('approved','paid')), 0) AS expenses,
  coalesce(sum(o.grand_total) FILTER (WHERE o.status NOT IN ('cancelled','refunded')), 0)
    - coalesce(sum(e.amount) FILTER (WHERE e.status IN ('approved','paid')), 0) AS net_profit,
  count(DISTINCT emp.id) AS employee_count
FROM branches b
LEFT JOIN orders o ON o.id IS NULL  -- orders not yet linked to branches; placeholder for future
LEFT JOIN expenses e ON e.branch_id = b.id
LEFT JOIN employees emp ON emp.branch_id = b.id
...
SELECT
  t.id, t.product_id, p.name AS product_name, p.sku,
  t.transaction_type, t.quantity, t.balance_after,
  t.reference_type, t.reference_id, t.notes,
  t.warehouse_id, wh.name AS warehouse_name,
  t.branch_id, br.name AS branch_name,
...
SELECT
  pb.id, pb.batch_number, pb.serial_number,
  p.name AS product_name, p.sku,
  s.name AS supplier_name,
  wh.name AS warehouse_name,
  br.name AS branch_name,
  pb.quantity_received, pb.quantity_remaining,
  pb.unit_cost, pb.unit_price,
  pb.expiry_date, pb.manufacture_date, pb.status,
  pb.received_at, pb.created_at
...
SELECT
  sa.id, sa.alert_type, sa.severity, sa.message, sa.quantity, sa.threshold,
...
LEFT JOIN warehouses wh ON wh.id = sa.warehouse_id
LEFT JOIN branches br ON br.id = sa.branch_id
...
-- Receive a purchase order: creates batches, updates inventory, logs transactions
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM purchase_order_items WHERE purchase_order_id = p_po_id LOOP
    ...
    UPDATE products SET cost = v_item.unit_cost, stock = stock + v_item.quantity WHERE id = v_product.id
    RETURNING stock INTO v_balance_after;
    ...
    -- If conflict (existing row), update quantity
...
CREATE OR REPLACE FUNCTION create_stock_alert(
  ...
  p_threshold integer DEFAULT NULL, p_warehouse_id uuid DEFAULT NULL,
...
BEGIN
  ...
  VALUES (p_product_id, p_warehouse_id, p_branch_id, p_alert_type, p_severity,
    coalesce(p_message, p_alert_type), p_quantity, p_threshold)
...
AS $$
BEGIN
  UPDATE stock_alerts SET is_resolved = true, resolved_at = now(), resolved_by = auth.uid()
  WHERE id = p_alert_id;
END;
$$;
