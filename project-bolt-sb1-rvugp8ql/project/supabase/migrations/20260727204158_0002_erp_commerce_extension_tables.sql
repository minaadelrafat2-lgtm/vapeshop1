/*
# ERP & Commerce extension tables

## Overview
Adds the remaining enterprise tables not present in migration 0001:
profiles, warehouses, inventory_transactions, stock_adjustments, stock_transfers,
payments, invoices, purchase_orders, purchase_order_items, expenses, reports,
activity_logs. All are normalized with foreign keys, indexes, and constraints.

## New Tables
- `profiles` — public user profile data (1:1 with auth.users), stores role + status
- `warehouses` — storage facilities (distinct from branches which are retail)
- `inventory_transactions` — immutable ledger of every stock movement
- `stock_adjustments` — manual stock corrections with reason codes
- `stock_transfers` — stock movements between warehouses/branches
- `payments` — payment records per order (supports partial payments)
...
- `reports` — saved/generated report definitions
- `activity_logs` — user activity trail (distinct from audit_logs which are system events)

## Security
- RLS enabled on every new table.
- `profiles`: owner read; self update; staff (is_staff) read all — used for admin user management.
- ERP tables (warehouses, inventory_*, stock_*, purchase_*, expenses, reports, activity_logs):
  staff-only CRUD via is_staff() gate.
- `payments`/`invoices`: owner read their own (via order.user_id); staff full access.
- Four separate policies per CRUD verb. Idempotent (DROP IF EXISTS before CREATE POLICY).
*/
...
CREATE TABLE IF NOT EXISTS inventory_transactions (
  ...
  CONSTRAINT chk_inv_tx_type CHECK (
    transaction_type IN ('purchase','sale','transfer_in','transfer_out','adjustment','return','reservation','release')
  )
);
...
CREATE TABLE IF NOT EXISTS stock_adjustments (
  ...
  warehouse_id uuid REFERENCES warehouses(id) ON DELETE SET NULL,
  branch_id uuid REFERENCES branches(id) ON DELETE SET NULL,
  adjustment_type text NOT NULL DEFAULT 'increment',
  quantity integer NOT NULL,
...
-- ============================================================
-- ACTIVITY LOGS (user activity trail)
-- ============================================================
