...
## Overview
Extends the existing schema (migrations 0001–0004) with the full ERP data model.
All changes are ADDITIVE — no columns dropped, no types changed, no tables renamed.
...
-- Drop old unique constraint if it exists, create a unique index instead
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'inventory_product_id_branch_id_key' AND contype = 'u'
  ) THEN
