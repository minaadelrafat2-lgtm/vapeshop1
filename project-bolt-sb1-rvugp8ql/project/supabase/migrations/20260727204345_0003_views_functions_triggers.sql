...
AS $$
  SELECT EXISTS (
    SELECT 1
    FROM profiles p
    JOIN employee_roles er ON er.employee_id = (
      SELECT e.id FROM employees e WHERE e.user_id = p.id LIMIT 1
    )
...
SELECT o.id, o.order_number, o.status, o.payment_status, o.grand_total,
  o.currency, o.placed_at, o.created_at,
...
SELECT p.id, p.name, p.slug, p.price, p.compare_at_price, p.stock,
  p.is_active, p.is_featured, p.is_best_seller, p.is_new_arrival, p.is_flash_sale,
  p.rating, p.review_count, p.sku,
...
SELECT date_trunc('day', o.placed_at) AS sale_date,
...
SELECT c.id, c.user_id, c.first_name, c.last_name, pr.email,
...
CREATE TRIGGER trg_recalc_rating_on_review_insert
...
CREATE TRIGGER trg_recalc_rating_on_review_update
  AFTER UPDATE ON product_reviews
  FOR EACH ROW EXECUTE FUNCTION recalc_product_rating();

DROP TRIGGER IF EXISTS trg_recalc_rating_on_review_delete ON product_reviews;
CREATE TRIGGER trg_recalc_rating_on_review_delete
...
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    'orders','payments','invoices','inventory','stock_adjustments',
    'stock_transfers','purchase_orders','employees','roles','permissions'
  ])
  LOOP
    EXECUTE format(
      'DROP TRIGGER IF EXISTS trg_audit_%s ON %I; '
      'CREATE TRIGGER trg_audit_%s AFTER INSERT OR UPDATE OR DELETE ON %I '
      'FOR EACH ROW EXECUTE FUNCTION audit_trigger_fn();', t, t, t, t
    );
  END LOOP;
END $$;
...
CREATE OR REPLACE FUNCTION log_activity(
  ...
  p_entity_id uuid DEFAULT NULL, p_metadata jsonb DEFAULT NULL
)
RETURNS void
LANGUAGE sql
SECURITY DEFINER
