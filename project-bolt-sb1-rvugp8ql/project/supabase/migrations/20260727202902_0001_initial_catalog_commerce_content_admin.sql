/*
# Initial schema — Vape & Smoking Company (premium commerce + ERP-ready foundation)

## Overview
This migration creates the complete data model for a premium vape & smoking e-commerce
platform with an ERP-ready foundation. Tables are organized into four logical groups:

1. CATALOG — products, categories, brands, variants, images, reviews
2. COMMERCE — customers, addresses, orders, order items, cart, wishlist, coupons
3. CONTENT — blog posts, blog categories, pages, store locations, careers, FAQ, newsletter
4. ADMIN/ERP — employees, branches, suppliers, inventory, roles, permissions, audit logs, notifications

The schema is intentionally normalized and extensible so future ERP modules
(purchasing, accounting, HR, logistics) can be added without restructuring.

## Tables created

### CATALOG
- `categories` — product categories (self-referencing parent_id for nested trees)
- `brands` — product brands/manufacturers
- `products` — sellable items
- `product_variants` — size/flavor/color variants of a product
- `product_images` — gallery images per product
- `product_reviews` — customer ratings & reviews
- `recently_viewed` — per-user recently viewed products
...
- `wishlist_items` — saved products per customer
- `cart_items` — guest + logged-in cart (user_id nullable for guest carts)

### CONTENT
- `blog_posts` — blog articles
- `blog_categories` — blog taxonomy
- `store_locations` — physical store locator entries
- `careers` — job postings
- `faq_entries` — FAQ items
- `newsletter_subscribers` — email newsletter signups

### ADMIN / ERP FOUNDATION
- `branches` — physical branches / warehouses
- `employees` — staff records (linked to auth.users)
...
- Catalog/content tables that are public-facing (products, categories, brands, blog, FAQ,
  store_locations, careers, product_reviews, newsletter) allow `anon, authenticated` SELECT
  so the storefront works without login. Writes are `authenticated` only with ownership checks.
- Customer-scoped tables (orders, addresses, wishlist, cart, recently_viewed, product_reviews)
  use `auth.uid()` ownership checks with `DEFAULT auth.uid()` on owner columns.
- Admin/ERP tables (employees, branches, suppliers, inventory, roles, permissions, audit_logs,
  notifications) are `authenticated` only — no anon access. (Admin UI will gate via auth + roles.)
- Four separate policies per CRUD verb (no `FOR ALL`).

## Important notes
1. Owner columns default to `auth.uid()` so inserts that omit the owner still satisfy WITH CHECK.
2. `cart_items.user_id` is nullable to support guest carts keyed by a session id.
3. All timestamps use `timestamptz DEFAULT now()`.
4. Idempotent: uses IF NOT EXISTS for tables/indexes and DROP IF EXISTS before CREATE POLICY.
5. Slug columns are unique for SEO-friendly URLs.
6. Money is stored as `numeric(12,2)` to avoid float rounding errors.
*/
...
-- products: public read (active only for anon), authenticated write
...
-- product_variants: public read, authenticated write
...
-- product_reviews: public read (approved), owner insert/update, owner delete
DROP POLICY IF EXISTS "anon_select_product_reviews" ON product_reviews;
CREATE POLICY "anon_select_product_reviews" ON product_reviews FOR SELECT
...
CREATE POLICY "auth_insert_product_reviews" ON product_reviews FOR INSERT
  TO authenticated WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "auth_update_product_reviews" ON product_reviews;
CREATE POLICY "auth_update_product_reviews" ON product_reviews FOR UPDATE
  TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "auth_delete_product_reviews" ON product_reviews;
CREATE POLICY "auth_delete_product_reviews" ON product_reviews FOR DELETE
...
-- orders: owner only (read), owner insert
...
-- coupons: public read (to validate at checkout), authenticated write
...
-- cart_items: owner (logged-in) read/update/delete; anon insert via session_id
...
-- store_locations: public read, authenticated write
DROP POLICY IF EXISTS "anon_select_store_locations" ON store_locations;
CREATE POLICY "anon_select_store_locations" ON store_locations FOR SELECT
  TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "auth_insert_store_locations" ON store_locations;
CREATE POLICY "auth_insert_store_locations" ON store_locations FOR INSERT
  TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "auth_update_store_locations" ON store_locations;
CREATE POLICY "auth_update_store_locations" ON store_locations FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "auth_delete_store_locations" ON store_locations;
CREATE POLICY "auth_delete_store_locations" ON store_locations FOR DELETE
  TO authenticated USING (true);
...
-- faq_entries: public read, authenticated write
DROP POLICY IF EXISTS "anon_select_faq_entries" ON faq_entries;
CREATE POLICY "anon_select_faq_entries" ON faq_entries FOR SELECT
  TO anon, authenticated USING (is_published = true);
DROP POLICY IF EXISTS "auth_insert_faq_entries" ON faq_entries;
CREATE POLICY "auth_insert_faq_entries" ON faq_entries FOR INSERT
  TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "auth_update_faq_entries" ON faq_entries;
CREATE POLICY "auth_update_faq_entries" ON faq_entries FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "auth_delete_faq_entries" ON faq_entries;
CREATE POLICY "auth_delete_faq_entries" ON faq_entries FOR DELETE
  TO authenticated USING (true);

-- newsletter_subscribers: public insert (signup), authenticated read
...
CREATE POLICY "auth_update_newsletter" ON newsletter_subscribers FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "auth_select_branches" ON branches FOR SELECT
  TO authenticated USING (true);
...
CREATE POLICY "auth_select_employees" ON employees FOR SELECT
  TO authenticated USING (true);
DROP POLICY IF EXISTS "auth_insert_employees" ON employees;
CREATE POLICY "auth_insert_employees" ON employees FOR INSERT
  TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "auth_update_employees" ON employees;
CREATE POLICY "auth_update_employees" ON employees FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "auth_delete_employees" ON employees;
CREATE POLICY "auth_delete_employees" ON employees FOR DELETE
  TO authenticated USING (true);
...
CREATE POLICY "auth_delete_roles" ON roles FOR DELETE
  TO authenticated USING (true);
...
CREATE POLICY "auth_insert_permissions" ON permissions FOR INSERT
  TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "auth_update_permissions" ON permissions;
CREATE POLICY "auth_update_permissions" ON permissions FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "auth_delete_permissions" ON permissions;
CREATE POLICY "auth_delete_permissions" ON permissions FOR DELETE
  TO authenticated USING (true);
...
CREATE POLICY "auth_insert_role_permissions" ON role_permissions FOR INSERT
  TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "auth_delete_role_permissions" ON role_permissions;
CREATE POLICY "auth_delete_role_permissions" ON role_permissions FOR DELETE
  TO authenticated USING (true);

-- employee_roles
DROP POLICY IF EXISTS "auth_select_employee_roles" ON employee_roles;
CREATE POLICY "auth_select_employee_roles" ON employee_roles FOR SELECT
  TO authenticated USING (true);
DROP POLICY IF EXISTS "auth_insert_employee_roles" ON employee_roles;
CREATE POLICY "auth_insert_employee_roles" ON employee_roles FOR INSERT
  TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "auth_delete_employee_roles" ON employee_roles;
CREATE POLICY "auth_delete_employee_roles" ON employee_roles FOR DELETE
  TO authenticated USING (true);

-- audit_logs: authenticated read, authenticated insert
