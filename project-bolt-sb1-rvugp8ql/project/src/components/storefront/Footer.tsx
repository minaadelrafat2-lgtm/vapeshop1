    ...
    <footer className="relative mt-24 border-t border-white/10 bg-ink-950/80">
      <div className="section py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 mb-12">
          <div className="col-span-2 md:col-span-2">
            ...
            <p className="text-sm text-ink-400 max-w-xs mb-6">
              Premium vape &amp; smoking essentials for the modern connoisseur. Crafted experiences, curated quality.
            </p>
            <div className="flex gap-3">
              {[Instagram, Twitter, Facebook, Youtube].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full glass flex items-center justify-center text-ink-300 hover:text-gold-300 hover:scale-110 transition" aria-label="Social link">
        ...
        <div className="grid sm:grid-cols-3 gap-4 py-6 border-t border-white/10">
          {[
            { icon: MapPin, label: '1 Liberty Plaza, New York, NY' },
            { icon: Phone, label: '+1 (800) 585-2937' },
            { icon: Mail, label: 'hello@luxe.co' },
          ].map((c, i) => (
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-ink-500">© {new Date().getFullYear()} LUXE Vape &amp; Smoking Co. All rights reserved.</p>
          <p className="text-xs text-ink-500">⚠️ For adults 21+. Products contain nicotine — a highly addictive substance.</p>
        </div>
      </div>
    </footer>
  );
