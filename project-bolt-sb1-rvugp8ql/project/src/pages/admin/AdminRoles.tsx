...
export default function AdminRoles() {
  const { rows, loading, remove } = useAdminTable<Role>('roles', 'hierarchy_level', true);
  ...
  return (
    <div>
      <AdminPageHeader title="Roles" subtitle={`${rows.length} roles in hierarchy`} action={<Button><Plus className="w-4 h-4" /> Add Role</Button>} />
        ...
        columns={[
          ...
          { key: 'actions', label: '', render: (r) => (
            <div className="flex gap-2">
              <button className="text-ink-400 hover:text-gold-300" disabled={r.is_system}><Pencil className="w-4 h-4" /></button>
              <button onClick={() => { if (!r.is_system) { remove(r.id); toast('Role deleted', 'info'); } }} className="text-ink-400 hover:text-error-500" disabled={r.is_system}><Trash2 className="w-4 h-4" /></button>
            </div>
          ) },
        ]}
      />
