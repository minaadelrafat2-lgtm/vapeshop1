  ...
  return (
    <article className="section py-10 max-w-3xl">
      ...
      <div className="aspect-[16/9] rounded-2xl overflow-hidden mt-8 mb-8 bg-ink-800">
        <img src={post.cover_image_url || blogImage(post.slug)} alt={post.title} className="w-full h-full object-cover" />
      </div>
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {related.map((r) => (
              <Link key={r.id} to={`/blog/${r.slug}`} className="group glass-card overflow-hidden card-hover">
                <div className="aspect-[16/10] overflow-hidden bg-ink-800"><img src={r.cover_image_url || blogImage(r.slug)} alt={r.title} className="w-full h-full object-cover group-hover:scale-105 transition" /></div>
