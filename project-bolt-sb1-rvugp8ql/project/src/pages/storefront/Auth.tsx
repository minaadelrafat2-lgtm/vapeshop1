...
export function SignUp() {
  ...
  if (user && !verifySent) return <Navigate to="/account" replace />;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (error) toast(error, 'error');
    else { setVerifySent(true); toast('Account created — verify your email to continue', 'success'); }
  };
    ...
    return (
      <div className="section py-16 max-w-md">
        <div className="glass-card p-8 text-center">
          ...
          <h1 className="text-2xl font-display font-semibold text-ink-50 mb-2">Verify Your Email</h1>
          <p className="text-ink-300 text-sm mb-6">We sent a verification link to <span className="text-gold-300">{email}</span>. Click it to activate your account.</p>
    ...
    <div className="section py-16 max-w-md">
      <div className="glass-card p-8">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <Input label="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
          <div className="relative">
            <Input label="Password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required hint="Min 6 characters" />
            <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3 top-9 text-ink-400 hover:text-gold-300" aria-label="Toggle password">
              {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
          </div>
