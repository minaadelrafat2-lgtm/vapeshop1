...
import { heroImage, blogImage } from '@/lib/images';
    ...
    <>
      ...
      <section className="relative min-h-[88vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroImage(0)} alt="" className="w-full h-full object-cover opacity-40" />
        ...
        <div className="relative section py-24">
          <div className="max-w-2xl animate-fade-up">
            <Badge color="gold" className="mb-5"><Sparkles className="w-3 h-3" /> New Season Collection</Badge>
            <h1 className="text-5xl md:text-7xl font-display font-bold text-ink-50 leading-[1.05] text-balance">
              The Art of <span className="text-gradient-gold">Fine Smoking</span>
            </h1>
            <p className="mt-6 text-lg text-ink-300 max-w-xl">
              Discover a curated collection of premium vape devices, artisan e-liquids, and refined smoking accessories — engineered for the discerning connoisseur.
            </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((c) => (
              <Link key={c.id} to={`/shop?category=${c.slug}`} className="group glass-card overflow-hidden card-hover aspect-square relative flex flex-col justify-end p-4">
                <div className="absolute inset-0 bg-dark-radial opacity-60 group-hover:opacity-100 transition" />
      ...
      <section className="section py-20">
        <div className="relative glass-card overflow-hidden min-h-[420px] flex items-center">
          <div className="absolute inset-0">
            <img src={heroImage(1)} alt="" className="w-full h-full object-cover opacity-30" />
            <div className="absolute inset-0 bg-gradient-to-r from-ink-950 to-transparent" />
          </div>
          <div className="relative p-10 md:p-16 max-w-xl">
            <Badge color="gold" className="mb-4">The LUXE Standard</Badge>
            <h2 className="text-3xl md:text-5xl font-display font-semibold text-ink-50 leading-tight text-balance">
              Crafted for those who <span className="text-gradient-gold">refuse ordinary</span>
            </h2>
            <p className="mt-4 text-ink-300">Every product in our collection is hand-selected by our experts and backed by our authenticity guarantee. No compromises. No counterfeits.</p>
            ...
            {posts.map((p) => (
              <Link key={p.id} to={`/blog/${p.slug}`} className="group glass-card overflow-hidden card-hover flex flex-col">
                <div className="aspect-[16/10] overflow-hidden bg-ink-800">
                  <img src={p.cover_image_url || blogImage(p.slug)} alt={p.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                </div>
      ...
      <section className="section py-20">
        <div className="glass-card p-10 md:p-16 text-center relative overflow-hidden">
          ...
          <div className="relative">
            <h2 className="text-3xl md:text-5xl font-display font-semibold text-ink-50 text-balance">Become a LUXE Member</h2>
            <p className="mt-4 text-ink-300 max-w-xl mx-auto">Join thousands of members enjoying exclusive pricing, early access, and rewards on every purchase.</p>
