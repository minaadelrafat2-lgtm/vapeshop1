      ...
      (row) =>
        `<tr>${columns
          .map((c) => {
            const raw = row[c.key];
            const val = c.format ? c.format(raw, row) : String(raw ?? '');
            return `<td style="padding:6px 8px;border:1px solid #ddd;color:#222;">${val}</td>`;
          })
          .join('')}</tr>`,
    )
    .join('');
...
export function exportToPdf(filename: string, title: string, columns: ExportColumn[], rows: Record<string, unknown>[]): void {
  ...
  const header = columns.map((c) => `<th style="text-align:left;padding:8px 12px;border-bottom:2px solid #1a1a2e;color:#1a1a2e;font-size:12px;text-transform:uppercase;">${c.label}</th>`).join('');
  const body = rows
    .map(
      (row, i) =>
        `<tr style="${i % 2 ? 'background:#f9fafb;' : ''}">${columns
          .map((c) => {
            const raw = row[c.key];
            const val = c.format ? c.format(raw, row) : String(raw ?? '');
            return `<td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;color #374151;font-size:13px;">${val}</td>`;
          })
          .join('')}</tr>`,
    )
    .join('');
